    ...
    <script type="text/javascript">
        ...
        function drawTrendlines() {
            ...
            data.addRows([
                ...
                <?php
/*
                $data1_line = "$date_output-01-01";
                $data2_line = "$date_output-12-31";




                    $Data_dt = date("Y-m-d", strtotime($Data));
                    $data1_dt = date("Y-m-d", strtotime($data1_line));
                    $data2_dt = date("Y-m-d", strtotime($data2_line));
                        ...
                        $date_outputD = $date_outputD[1];
                                            ...
                                            $total_mês = $total_mês + $Valor;

                                            echo"[$date_outputD,  $total_mês, $cont, $cont],\n         ";
...
$despmaisinvest = 0;
   ...
   // $query_despesa = "SELECT Valor FROM despesa WHERE Usuario_ID_Usuario LIKE $ID AND Data_Venc BETWEEN $data[$i] ORDER BY Valor DESC";
    ...
    $query_despesa = "SELECT sum(parcela.valor_parcela) FROM despesas INNER JOIN parcela ON parcela.Id_despesa = despesas.Id_despesa WHERE valido LIKE 's' AND ID_Usuario LIKE '$ID' AND Data_parcela BETWEEN '$data1_dp[$i]' AND '$data2_dp[$i]' ORDER BY Data_parcela ASC";
    ...
    while ($linha = mysql_fetch_array($seleciona_despesa)) {
        ...
        $query_receita = "SELECT sum(valor) FROM receita WHERE ID_Usuario LIKE $ID AND data BETWEEN '$data1_dp[$i]' AND '$data2_dp[$i]' AND valido LIKE 's' ORDER BY valor DESC";
        ...
        while ($linha2 = mysql_fetch_array($seleciona_receita)) {
            ...
            //echo"/*$Tot_Rec_mes*/\n";
                ...
                $query_investimento = "SELECT sum(Valor) FROM investimento WHERE ID_Usuario LIKE $ID AND Data BETWEEN '$data1_dp[$i]' AND '$data2_dp[$i]' AND valido LIKE 's' ORDER BY Valor DESC";
...
$jscommand
    ";
            ...
            var options = {
                ...
                title: 'Despesas x receitas baseadas em seus dados',
